...
export function PostPreview({
  ...
  return (
    <div className="post-preview">
      ...
      <h3 className="text-3xl mb-3 leading-snug text-blue-500">
        <Link className="hover:underline" href={`/posts/${slug}`}>
          {title}
        </Link>
      </h3>
      <div className="text-lg mb-4 text-blue-200">
        <DateFormatter dateString={date} />
      </div>
      <Link className="hover:underline" href={`/posts/${slug}`}>
        <p className="text-lg leading-relaxed mb-4 text-blue-400">{excerpt}</p>
      </Link>
      <div className="author flex items-center">
        ...
        />
        <span className="text-blue-300">{authorName}</span>
      </div>
    </div>
  );
}
